...
for (var i = 0; i < 8; i++) {
  ...
  document.getElementById(btnNo[choose1]).style.backgroundImage =
    "url(" + image[i] + ")";
  btnNo.splice(choose1, 1);
  var choose2 = Math.floor(Math.random() * btnNo.length);
  document.getElementById(btnNo[choose2]).style.backgroundImage =
    "url(" + image[i] + ")";
...
function game(clicked_id) {
  if (start == true) {
    ...
    var url = document.getElementById(
      document.getElementById(clicked_id).lastElementChild.id
